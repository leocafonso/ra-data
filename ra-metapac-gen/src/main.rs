...
fn main() -> anyhow::Result<()> {
    ...
    for entry in fs::read_dir(chips_dir).context(format!("failed to read chips directory: {:?}", chips_dir))? {
        ...
        if path.extension().map_or(false, |ext| ext == "json") {
            ...
            generate_chip_pac(&chip, &chip_dir, &mut generator)?;
...
fn generate_chip_pac(chip: &Chip, chip_dir: &Path, generator: &mut Generator) -> anyhow::Result<()> {
    ...
    // Maps type_name -> (file_path, type_name) e.g., "gpt" -> ("gpt_v1", "gpt")
    let mut register_modules: BTreeMap<String, String> = BTreeMap::new();
    for peri in &chip.peripherals {
        let file_name = format!("{}_{}", peri.peri_type.to_lowercase(), peri.version.to_lowercase());
        let type_name = peri.peri_type.to_lowercase();
        register_modules.insert(type_name, file_name);
    }

    // Generate #[path] module references to register definitions
    // Use type-only module names (e.g., "gpt" not "gpt_v1")
    for (type_name, file_name) in &register_modules {
        writeln!(file, "#[path = \"../../peripherals/{}.rs\"]", file_name)?;
        writeln!(file, "pub mod {};", type_name)?;
    }
    writeln!(file)?;

    // Generate top-level const declarations for peripheral instances
    for peri in &chip.peripherals {
        let type_mod = peri.peri_type.to_lowercase();
        let block_name = heck::AsPascalCase(&peri.block).to_string();
        writeln!(file, "pub const {}: {}::{} = unsafe {{ {}::{}::from_ptr(0x{:08x}usize as _) }};", 
            peri.name, type_mod, block_name, type_mod, block_name, peri.address)?;
    }
    writeln!(file)?;
    ...
    // Generate PINS constant for unique GPIO pins on this chip
    writeln!(shared_content, "const PINS: &[ChipPin] = &[")?;
    for pin in &chip.pins {
        writeln!(shared_content, "    ChipPin {{")?;
        writeln!(shared_content, "        name: \"{}\",", pin.name)?;
        writeln!(shared_content, "    }},")?;
    }
    writeln!(shared_content, "];")?;
...
// Note: resolve_block_name is no longer needed since we use peri.block directly
