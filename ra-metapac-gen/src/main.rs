...
use anyhow::Context;
use ra_data_types::{Chip, Peripheral};
...
fn generate_chip_pac(chip: &Chip, chip_dir: &Path, block_map: &BTreeMap<String, String>, generator: &mut Generator) -> anyhow::Result<()> {
    ...
    for peri in &chip.peripherals {
        let mod_name = format!("{}_{}", peri.peri_type.to_lowercase(), peri.version.to_lowercase());
        let block_name = resolve_block_name(peri, &mod_name, block_map);
...
fn resolve_block_name(peri: &Peripheral, mod_name: &str, block_map: &BTreeMap<String, String>) -> String {
    block_map
        .get(mod_name)
        .cloned()
        .unwrap_or_else(|| heck::AsPascalCase(&peri.peri_type).to_string())
}
