...
/// Extract the short family identifier from the full family name.
/// e.g., "R7FA6E2" -> "ra6e2", "R7FA2E1" -> "ra2e1", "R7KA8P1" -> "ra8p1"
fn extract_family_short(family: &str) -> String {
    // Family format: R7FA<series><group> or R7KA<series><group> or R8FA<series><group>
    // e.g., R7FA6E2, R7FA2E1, R7KA8P1
    // Note: R7KA prefix indicates dual-core variants, but the family is still "ra8p1"
    let family_lower = family.to_lowercase();
    
    if family_lower.starts_with("r7fa") {
        // R7FA6E2 -> ra6e2
        format!("ra{}", &family_lower[4..])
    } else if family_lower.starts_with("r7ka") {
        // R7KA8P1 -> ra8p1 (K indicates dual-core, but family remains "ra")
        format!("ra{}", &family_lower[4..])
    } else if family_lower.starts_with("r8fa") {
        // R8FAxxxx -> ra8xxxx (hypothetical future format)
        format!("ra{}", &family_lower[4..])
    } else {
...
fn generate_chip_pac(chip: &Chip, chip_dir: &Path, block_map: &BTreeMap<String, String>, generator: &mut Generator) -> anyhow::Result<()> {
    ...
    // Collect unique register modules needed by this chip
    let mut register_modules: BTreeMap<String, String> = BTreeMap::new();
    for peri in &chip.peripherals {
        ...
        register_modules.insert(mod_name.clone(), block_name);
    }

    // Generate #[path] module references to register definitions
    for (mod_name, _block_name) in &register_modules {
        writeln!(file, "#[path = \"../../peripherals/{}.rs\"]", mod_name)?;
        writeln!(file, "pub mod {};", mod_name)?;
    }
    writeln!(file)?;

    // Generate per-kind alias modules when the kind maps to a single register module.
    let mut kind_modules: BTreeMap<String, String> = BTreeMap::new();
    let mut duplicate_kinds: HashSet<String> = HashSet::new();
    for peri in &chip.peripherals {
        let kind = peri.peri_type.to_lowercase();
        let mod_name = format!("{}_{}", peri.peri_type.to_lowercase(), peri.version.to_lowercase());
        match kind_modules.get(&kind) {
            Some(existing) if existing != &mod_name => {
                duplicate_kinds.insert(kind.clone());
            }
            Some(_) => {}
            None => {
                kind_modules.insert(kind, mod_name);
            }
        }
    }
    for kind in &duplicate_kinds {
        kind_modules.remove(kind);
    }
    for (kind, mod_name) in &kind_modules {
        writeln!(file, "pub mod {} {{", kind)?;
        writeln!(file, "    pub use super::{}::*;", mod_name)?;
        writeln!(file, "}}")?;
    }
    if !kind_modules.is_empty() {
        writeln!(file)?;
    }

    // Generate top-level const declarations for peripheral instances
    for peri in &chip.peripherals {
        let mod_name = format!("{}_{}", peri.peri_type.to_lowercase(), peri.version.to_lowercase());
        let block_name = register_modules.get(&mod_name).unwrap();
        writeln!(file, "pub const {}: {}::{} = unsafe {{ {}::{}::from_ptr(0x{:08x}usize as _) }};", 
            peri.name, mod_name, block_name, mod_name, block_name, peri.address)?;
    }
    writeln!(file)?;

    // Generate peripherals module with singleton types for embassy ownership
    ...
    for (i, peri) in chip.peripherals.iter().enumerate() {
        ...
        writeln!(file, "        type Target = super::{}::{};", mod_name, block_name)?;
        writeln!(file, "        fn deref(&self) -> &Self::Target {{")?;
        writeln!(file, "            &super::{}", peri.name)?;
        writeln!(file, "        }}")?;
        writeln!(file, "    }}")?;
        ...
        writeln!(file, "            &crate::metadata::METADATA.peripherals[{}]", i)?;
        writeln!(file, "        }}")?;
        writeln!(file, "    }}")?;
    }
    ...
    // Extract short family identifier for cfg flags (e.g., "R7FA6E2" -> "ra6e2")
    let family = extract_family_short(&chip.family);

    writeln!(shared_content, "const FAMILY: &str = \"{}\";", family)?;
    writeln!(shared_content, "const CORE: &str = \"{}\";", chip.core)?;
    writeln!(shared_content, "const INTERRUPT_COUNT: usize = {};", interrupt_count)?;

    writeln!(shared_content, "const MEMORY: &[MemoryRegion] = &[")?;
    ...
    writeln!(shared_content, "const PERIPHERALS: &[Peripheral] = &[")?;
    for peri in &chip.peripherals {
    ...
    writeln!(shared_content, "const EVENTS: &[Event] = &[")?;
    for irq in &chip.interrupts {
    ...
    writeln!(shared_content, "const PACKAGES: &[Package] = &[")?;
    for pkg in &chip.packages {
    ...
    writeln!(file)?;
    writeln!(file, "pub static METADATA: Metadata = Metadata {{")?;
    writeln!(file, "    name: \"{}\",", chip.name)?;
    writeln!(file, "    family: FAMILY,")?;
    writeln!(file, "    core: CORE,")?;
    writeln!(file, "    memory: MEMORY,")?;
    writeln!(file, "    peripherals: PERIPHERALS,")?;
    writeln!(file, "    events: EVENTS,")?;
    writeln!(file, "    packages: PACKAGES,")?;
    writeln!(file, "    interrupt_count: INTERRUPT_COUNT,")?;
    writeln!(file, "}};")?;
